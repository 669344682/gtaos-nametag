...
CreateThread(function()
    while true do
        --print("client: 1")
        Wait(1000)
        TriggerServerEvent('qb-nametag:server:GetPlayers')
    end
end)

RegisterNetEvent('qb-nametag:client:ShowNametag',function(players)
    for _, player in ipairs(players) do
        local playeridx = GetPlayerFromServerId(player.id)
        local ped = GetPlayerPed(playeridx)
        local gamename = player.name

        if ped then
            --local nametag = 'ID: '..player.id..' ('..player.joblabel..') | '..gamename
            local nametag = 'ID: '..player.id..' | '..gamename
            --print("joblabel: "..player.joblabel)
            RemoveMpGamerTag(player.id)
            local Tag = CreateFakeMpGamerTag(ped, nametag, false, false, "", false)
